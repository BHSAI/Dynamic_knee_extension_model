...
for i=1:n
    ...
    [~,S,~,~,~,~,~]=eval_pH(params,iemg_profile,T_cycles, cut_off, H_set(i), met_dyn_set);
    force(:,i)=S(:,6);
    Pi_lvl(:,i)=S(:,1);
    ADP_lvl(:,i)=S(:,2);
    PCr_lvl(:,i)=S(:,3);
...
exportgraphics(figure(5),fullfile('figure_4_subplots','figure_4_b.pdf'),'BackgroundColor','w','Resolution',300,'ContentType','vector');